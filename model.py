...
class Actor_Critic_Models():
    ...
    def __init__(self, n_agents, state_size=24, action_size=2, seed=0, lr_critic = 1e-3, weight_decay=0.0):
        """
        Params
        ======
            ...
            seed (int): random seed
            lr_critic(double) : Learning rate of the critic
            weight_decay(double) : Weight decay of the optimizer
        """
        
        self.actors_local = [ActorModel(state_size, action_size, seed).to(device) for i in range(n_agents)]
        self.actors_target = [ActorModel(state_size, action_size, seed).to(device) for i in range(n_agents)]
