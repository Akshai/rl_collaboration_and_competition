...
class MADDPG():
    ...
    def __init__(self, action_size=2, seed=0, 
        ...
        models = model.Actor_Critic_Models(n_agents=n_agents)
        self.agents = [DDPG(i, models) for i in range(n_agents)]
    ...
    def save_agents(self):
        ...
        for i, agent in enumerate(self.agents):
            torch.save(agent.actor_local.state_dict(),  f"checkpoints/checkpoint_actor_agent_{i}.pth")
            torch.save(agent.critic_local.state_dict(), f"checkpoints/checkpoint_critic_agent_{i}.pth")
    
    def load_weights(self):
        # save models for local actor and critic of each agent
        for i, agent in enumerate(self.agents):
            agent.actor_local.load_state_dict(torch.load(f"checkpoints/checkpoint_actor_agent_{i}.pth", map_location='cpu'))
            agent.critic_local.load_state_dict(torch.load(f"checkpoints/checkpoint_critic_agent_{i}.pth", map_location='cpu'))
